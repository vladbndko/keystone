...
There are three different types of access control that can be configured as properties of the `access` object, for any given list:

- [`operation`](#operation), which lets you check the `context` and `session` to decide if the request should be allowed access to the list, prior to the GraphQL query being executed.
- [`filter`](#filter), which lets you provide a [filter](./filters) that will be applied to the respective database queries while resolving the GraphQL query.
- [`item`](#item-mutations-only), which lets you check the `context`, `session` and any resolved data to decide if the request should be allowed access before any mutative actions are performed.

Each of these types of access control are applied before any [hooks](./hooks) are executed.
